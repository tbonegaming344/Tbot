...
module.exports = {
	name: `randomnt`,
	aliases: [`ntrandom`, `randomdecknt`, `neptunarandom`, `randomneptuna`, `randomizednt`, `ntrandomized`, `ntrandomdeck`, `rnt`],
	category: `Neptuna(NT)`,
	run: async(client, message, args) => {
		let [result]= await db.query(`SELECT * FROM ntdecks WHERE deckinfo = 'image link:'`)
		const deck = [
//NT TBOT DB
`${result[0].agraves}`,
`${result[0].antiagor}`,
