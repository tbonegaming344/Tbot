...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const cheaper = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("ifb")
        .setLabel("🔙")
        .setStyle(ButtonStyle.Primary),
    ...
    const cj = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("cpack")
        .setLabel("🔙")
        .setStyle(ButtonStyle.Primary),
    ...
    const uno = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("splimps")
        .setLabel("🔙")
        .setStyle(ButtonStyle.Primary),
    ...
    let embed = new EmbedBuilder()
      ...
      .setFooter({
        text: `To view the Impfinity decks please use the commands listed above or click on the buttons below!
Note: Impfinity has ${decks.length - 3} total decks in Tbot`,
    ...
    collector.on("collect", async (i) => {
      ...
      if (i.customId == "lm") {
        await i.update({ embeds: [lme], components: [lm] });
      }
      if (i.customId == "pc") {
        await i.update({ embeds: [pc], components: [ifp] });
      }
