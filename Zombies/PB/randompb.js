...
module.exports = {
	name: `randompb`,
	aliases: [`pbrandom`, `randomdeckpb`, `pbrandomdeck`, `randomizedpbdeck`, `pbrandomizeddeck`, `rpb`],
	category: `Professor Brainstorm(PB)`,
	run: async(client, message, args) => {
		let [result] = await db.query(`select * from pbdecks where deckinfo = 'image link:'`)
		const deck = [
//PB TBOT DB
`${result[0].barrelster}`,
...
//PB WEB DB 
	...
	.setDescription(`Here is your random Professor Brainstorm(PB) Deck ${message.author}`)
		.setColor("Random")
