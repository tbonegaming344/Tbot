...
module.exports = {
	name: `randomct`,
	aliases: [`ctrandom`, `randomizeddeckct`, `citronrandom`, `randomcitron`, `randomizedeckct`, `ctrandomizedeck`],
	category: `Beta-Carrotina/Citron`,
	run: async(client, message, args) => {
		let [result] = await db.query("SELECT * FROM ctdecks WHERE deckinfo = 'image link:'")
		const deck = [
			//CT TBOT DB 
			`${result[0].anti}`,
			`${result[0].budgetct}`,
