	...
	const m = await	message.channel.send({embeds: [ embed ], components: [row] } )
		...
		collector.on('collect', async i  => {
			if (i.customId == 'helprb')  {
				await i.update({embeds: [helprb], components: [test]} )
			}
			if(i.customId == 'helprbr'){
					await i.update({embeds: [helprb], components: [test]} )
			}
			if(i.customId == 'bol'){
